...
import { createTestOffer, getRandomBigInt, ONE_SOL } from "./EscrowDapp.test-helpers";
...
const REFUND_OFFER_ERROR =
  "4XLs7eTjECzLwby7D9SwWbSZa9BjFvMrDZb7zEaVNC5k.RefundOffer: A has one constraint was violated";
...
describe("Escrow", () => {
  ...
  describe("takeOffer", () => {
    ...
    test("fails when trying to take an already refunded offer", async () => {
      const { offer, vault } = await createTestOffer({
        connection,
        maker: alice,
        tokenMintA,
      ...
      // Refund the offer first (vault gets closed)
      ...
      try {
        ...
        // Anchor deserialization failure
        assert(
          error.message.includes("The program expected this account to be already initialized"),
          `Expected account initialization failure but got: ${error.message}`,
        );
      }
    });
  ...
  describe("refundOffer", () => {
    ...
    test("fails when trying to refund an already refunded offer", async () => {
      const { offer, vault } = await createTestOffer({
        connection,
        maker: alice,
        tokenMintA,
      ...
      // Try refunding again → should fail due to vault account not being initialized
      try {
        await connection.sendTransactionFromInstructions({
          feePayer: alice,
          instructions: [refundOfferInstruction],
        });
        assert.fail("Expected the second refund to fail but it succeeded");
      } catch (thrownObject) {
        const error = thrownObject as ErrorWithTransaction;
